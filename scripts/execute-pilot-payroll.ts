 ...
 *
 * Runs: create_run → escrow_payout → finalize_run → transfer run
 * all in a single PTB.
 *
 * Usage:
 *   RELAYER_PRIVATE_KEY=suiprivkey1... npx tsx scripts/execute-pilot-payroll.ts
 */
...
async function main() {
  ...
  if (!sk) {
    ...
    console.error('Usage: RELAYER_PRIVATE_KEY=suiprivkey1... npx tsx scripts/execute-pilot-payroll.ts');
    process.exit(1);
  }
  ...
  console.log(`Secret:   ${SECRET_HEX}`);
  console.log(`ID Hash:  ${ID_HASH_HEX}`);
  console.log('');
  ...
  // Step 1: create_run → returns PayrollRun
  ...
  // Step 3: escrow_payout (takes &mut PayrollRun)
  tx.moveCall({
  ...
  // Step 4: finalize_run (takes &mut PayrollRun)
  tx.moveCall({
    target: `${PACKAGE_ID}::payroll::finalize_run`,
    arguments: [run, tx.object(ADMIN_CAP)],
  });

  // Step 5: Transfer the run back to sender (required: PayrollRun has key ability)
  ...
  if (result.objectChanges) {
    ...
    for (const obj of created) {
      const o = obj as any;
      const typeName = o.objectType?.split('::').pop() || o.objectType;
      console.log(`  ${typeName}: ${o.objectId}`);
    }
    console.log('');
  }
  ...
  fs.writeFileSync('/tmp/veil-pilot-result.json', JSON.stringify(pilotData, null, 2));
